...
DBIx::Simple::Procedure - An Alternative To DB Stored Procedures using DBIx::Simple
...
# depreciated [now sports a commands dispatch table] - our @sqlfile_commands = ('capture', 'include', 'execute', 'replace', 'storage', 'proceed');
...
Here is an example of how to setup and process a (sql) text file.
    ...
    # DBIx::Simple::Procedure uses L<DBIx::Simple> as it base class and provides an accessor through the DBIx::Simple::Procedure->{dbix} hash reference.
...
sub _load_commands {
    ...
    #! skipif: evaluates the statement passed (perl code) for truth, if true, it continues if false it skips to the next proceed command or until the end of the sql file.
    $self->{commands}->{proceed} = sub {
        my ($statement, @parameters) = @_;
        $statement =~ s/\?/$_/ foreach @parameters;
    ...
    $self->{commands}->{declare} = sub {
        my ($statement, @parameters) = @_;
        $self->{processing}->{resultset} = $self->_execute_query($statement, @parameters);
        $self->{processing}->{custom_parameters} = $self->{processing}->{resultset}->hash;
    };
...
sub _execute_query {
    ...
    return $self->{dbix}->query( $statement, @parameters ) or die ref($self) . " - sql file $self->{file} failed execution of command (capture) and statement (".substr($statement,0,10)."...) at $properties[1] on line $properties[2], " . $self->{dbix}->error;
}
...
sub process_queue {
    ...
    if (@{$self->{cmds}}) {
        ...
        die ref($self) . " - Could'nt process sql file $self->{file} from $properties[1] on line $properties[2], file has no commands to process.";
    }
    return $self;
}
...
sub _processor {
    ...
    next if $self->{processing}->{skip_switch} && $command ne "proceed";
...
sub _parse_sqlfile {
    ...
    $self->{file} = $sqlfile;
    open (SQL, "$self->{path}$sqlfile") || die ref($self) . " - Could'nt open $self->{path}$sqlfile sql file from $properties[1] on line $properties[2], $!.";
    push @lines, $_ while(<SQL>);
    close SQL || die ref($self) . " - Could'nt close $self->{path}$sqlfile sql file from $properties[1] on line $properties[2], $!.";
    ...
    foreach my $statement (@statements) {
        if (grep ( $statement->{command} eq $_, @{$self->{select_required}})) {
            if (lc($statement->{statement}) !~ /^(\s+)?select/) {
                die ref($self) . " - Validation of the sql file $self->{file} failed within $properties[1] on line $properties[2]. The command ($statement->{command}) can only be used with a SQL (select) statement.";
            }
        }
    }
}
